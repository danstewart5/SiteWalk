...
1. Open https://sitewalk-app.netlify.app (live front door)
...
- CORS is locked to the origins listed in `ALLOWED_ORIGINS` in `worker.js` — currently `https://sitewalk-app.netlify.app` and `https://danstewart5.github.io` (the old GitHub Pages copy, which stops serving once the repo is private; harmless to leave in). Anything else (localhost, `file://`, a different Netlify preview/branch subdomain) will fail CORS and look like a dead network. To reopen for local/dev, set `ALLOWED_ORIGINS` to `['*']` in `worker.js`.
