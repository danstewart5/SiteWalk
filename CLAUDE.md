...
## Role views — "View as" (2026-09-22, cache `sitewalk-v62`)

A four-button bar at the top of Home (`.home-role-btn[data-role]`, persisted `swRole`, applied as `data-role` on `#tab-home`): **Admin** (`all`, default — the unchanged Build/Hold dial scene), **Manager**, **Bookkeeper**, **Developer**. Any non-Admin role hides the mode toggle, dial scene and drilldown and shows `#homeRoleView`, rendered by `renderRoleView()` in `walk.js` from the `ROLES` config. Each role lists `sections`, drawn by `roleSectionHtml()` off one shared `roleStats()` snapshot, then shortcut buttons into existing `tab-*` panels + disabled "Coming soon" placeholders. Re-renders on every `renderFlagBar()` and whenever Home is shown.

Sections: **flags** (`computeProblemFlags()` — every problem tagged with the roles that care and the tab that fixes it; red first: over budget, RFIs unanswered 7+ days, safety incidents (not near-miss/hazard) in 14 days, missed clock-outs, arrears; yellow: spending >10 pts ahead of build %, delays in Daily Log this week, pending COs with $, open punch, pending submittals, no daily log in 2+ days, progress not updated in 14 days, renewals in 60 days, vacant units), **build** (stage + % complete bar vs budget-spent bar, editable inline), **delays** (last 5 Daily Log entries with a delay), **field**, **money** (budget, spent, remaining, approved COs, pending CO exposure, revised contract), **rentals**, **books**.

- **Manager** — Start Walk / Report buttons; flags, build, delays, field, money.
- **Developer** — flags, build, money ("The big dollars"), delays, rentals — the whole picture; effectively a first cut of the Ch.8 Developer Dashboard (the disabled Ch.8 button in the Admin drilldown is still there).
- **Bookkeeper** — money flags only, then books.

**Build progress is the one new piece of data** (`swBuildProgress` = `{stage, pct, updated}`, `BUILD_STAGES` list) — nothing else in the app tracks schedule progress, so it's set by hand from the Manager/Developer view; there's no schedule/milestone engine. One job only (the app has one job site).

**A lens, not access control** — no login, anyone can tap any role; Setup is only reachable from Admin. And because everything is still per-device `localStorage`, a bookkeeper on their own phone sees their own empty data — role views only become truly useful once cross-device sync (parked Firebase) exists. Tested with headless Chromium at 390px width (all three views with seeded data, flag → tab links, progress save, no horizontal scroll, no JS errors); not phone-tested.
