...
**Nav restructured to a Home dashboard, 2026-09-19** (cache `sitewalk-v47`) — the top nav bar is now just a single 🏠 Home button (`#topNav` in `index.html`), not a flat row of chapter tabs. `tab-home` is the new default landing panel (`TABS[0]` in `walk.js`, `showTab('home')` at the end of the `window.addEventListener('load', ...)` init block). It shows two quick-action buttons (`#homeStartWalkBtn` calls `showTab('walk')` + `window.startWalk()` directly; `#homeGenerateReportBtn` calls `showTab('report')` + `window.generateReport()` directly — no need to enter a chapter tab first) plus four Procore-style top-level block accordions (`.home-block[data-block="preconstruction|pm|rm|fm"]`, toggled open/closed by `.home-block-header` clicks adding/removing `.expanded`). Each block's `.home-block-chapters` list reveals the real chapter buttons one layer behind — `.home-chapter-btn[data-tab]`/`.home-admin-btn[data-tab]` still just call the pre-existing `showTab(name)` against the same `tab-*` panels below, nothing about the panels themselves changed. `disabled` chapter buttons with no `data-tab` (Land Acquisition, Ch.6 Land-to-Contract Pipeline/Victoria Land, Ch.9 Price-the-House, Ch.5 Invoicing, Ch.7 Consulting Business Rollout, Ch.8 Developer Dashboard) are placeholders — none of that functionality exists in the live app; Ch.6/Ch.9's real reference material is the parked `current/pipeline.ts`/`pipeline-view.tsx`, not anything live.

Block mapping (chapter numbers are the user's own external roadmap numbering, not something derived from this repo):
- **Preconstruction** — Land Acquisition (placeholder) + Ch.6 Land-to-Contract Pipeline/Victoria Land (placeholder) + Ch.9 Price-the-House (placeholder)
- **Project Management** — Ch.1 Walk-Around (`walk`, live) + Ch.3 Punch List & Estimating (`punch`, live — punch list only, no estimating tool yet) + Change Orders/RFIs/Submittals/Safety Log/Drawing References/Drawings (all live, not individually chapter-numbered by the user but grouped here as construction-execution features)
- **Resource Management** — Ch.2 GPS Clock-In (`clock`, live as manual clock in/out only — geofencing still parked, see below) + Ch.4 Trade Routing (`contacts`, live only as manual per-trade email/SMS contacts — the real routing engine, `routeItem`, is still parked in `current/routing.ts`) + Daily Log (`dailylog`, live)
- **Financial Management** — Ch.5 Invoicing (placeholder, nothing live) + Job Cost Dashboard (`jobcost`, live — budget vs actual, not literal invoicing)
- **Admin & Overview** (outside the four blocks, per the user's brief) — Ch.7 Consulting Business Rollout (placeholder) + Ch.8 Developer Dashboard (placeholder, distinct from the live Open Items Dashboard) + Open Items Dashboard (`dashboard`, live) + Report (`report`, live) + Setup (`setup`, live)
