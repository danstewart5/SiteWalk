...
## Photo tag suggestions + urgency-first report (2026-09-25, cache `sitewalk-v74`)

Built on top of the Chapter 1 walk engine and the 2026-09-23 report above, not replacing either. No written brief was in the repo; this was built from the request text.

- **Photo tags.** Every walk photo (voice snap, Snap button, upload) goes through `requestPhotoTag()` → Worker `POST /tag-photo` (image shrunk to 768px long edge, JPEG 0.6, plus the voice trade and what was being said as hints). The reply `{kind: trade|safety|quality, trade, severity, confidence, observation}` is stored as `photo.suggestion`. Nothing changes until the user acts. The Walk screen only shows a one-line badge (`#photoTagQueue`, "📷 N photos need a look →", opens Review) — no dropdowns over the live camera mid-walk (changed in v73 after review; v72 put a full card there). The cards (`photoTagCard()`) live in Walk → Review (`#photoTagReviewList`), with trade / tag / urgency dropdowns prefilled from the suggestion, **Confirm** and **Keep as tagged**. The Review badge counts these too. A suggestion that wouldn't change anything is stored as `status: 'agrees'` and not shown (the call is still paid for; the issue gate below is what cuts cost). Unconfirmed suggestions never touch an item's trade/severity or the report. Severity only ever goes up from a photo (`maxSeverity`); a Low photo can't lower a Critical sentence.
  - Confirm (`confirmPhotoTag()`) writes `photo.tag = {kind, trade, severity, by: 'ai'|'user'}` (`by: 'user'` = overridden) and retags the photo. If the photo is linked to an item, `applyPhotoTagToItem()` sets the item's trade (`tradeBy: 'photo'`), raises its severity, and for safety/quality sets `photoFlag`. A linked **Review** item confirmed as safety/quality is filed straight to the Safety & Quality log. An unlinked safety/quality photo becomes its own Safety & Quality entry (`via: 'photo'`). A photo still held for its sentence (`holdForId`) keeps the tag and applies it when it links.
  - **Cost gate (v74), under the Setup switch:** a photo is sent to `/tag-photo` only once it's tied to an issue: linked to an item the sentence filter filed (punch/CO/RFI/safety/quality) or put in Review. A snap inside a neutral sentence ("heading upstairs, take a photo") is skipped for good and keeps its voice trade. A Snap-button or bare "take a photo" shot is sent if it links to an item within the existing 60 s window (before or after); otherwise it's skipped after 60 s. The decision is made in `gatePhotoTag()`/`skipPhotoTag()`/`waitForPhotoIssue()` before any resize or upload (`photo.tagGate`: `waiting`/`sent`/`skipped`). Sent/skipped counts: LS `swPhotoTagStats` (whole phone) and `currentWalk.photoTags` (per walk), shown under the Setup toggle (`#photoTagStats`) and logged to the console. Side effect: an unlinked safety/quality photo can no longer become its own Safety entry, because unlinked photos are never sent.
  - Pure fetch: it never touches the camera tracks, mic or `recognition`. Needs a Worker URL; toggle in Setup (`#photoTagToggle`, LS `swPhotoTagEnabled`, default on). **The Worker must be redeployed** for `/tag-photo`; an old Worker just fails the call and photos keep their voice trade.
- **Urgency.** Items carry `severity`: `critical`/`high`/`medium`/`low`. Set at capture as the highest of: `scoreSeverity()` on the words (`CRITICAL_RE`/`HIGH_RE`/`LOW_RE`; safety defaults to high), the Worker's `/classify` `severity` (new field), and a confirmed photo tag. Older items without one are scored from their text on the fly (`itemSeverity()`). Every punch/CO/RFI, Safety and Review card has an "Urgency: X" button that steps Low → Medium → High → Critical (`severityBy: 'user'`).
- **Report order.** A "Most Urgent First" block tops the report: every open Critical/High item across punch, COs, RFIs, Safety & Quality and Review, worst first, with full details and photos. **Each item is listed once**: `reportItemSection(..., pinned)` skips items already in that block and the heading says "N listed above" (v73; v72 repeated them, doubling the printout). The rest of each section is sorted by `byUrgency()`: open before closed, then severity, then capture order (was trade, then time). Each row has a coloured urgency badge; rows show "Photo flagged: Safety" / "Trade from photo" when a photo tag set them. Unlinked photos are sorted by their tag's urgency. The Walk Summary sorts within each trade the same way. The transcript/walk log stays in capture order (it's the record).

Tested with headless Chromium at 390px, fake camera, mocked Worker: suggestion card, confirm, override (quality → safety, new safety entry), agree (no card), held-photo-in-sentence tag, Review item + hazard photo → Safety log, word-based and Worker severity, manual urgency cycle, report order, reload persistence. No page errors, no horizontal scroll. **Not phone-tested, and `/tag-photo` hasn't been run against the real API.**
