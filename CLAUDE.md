...
**Nav restructured to a Home dashboard, 2026-09-19, revised same day to seven pillars** (cache `sitewalk-v48`) — the top nav bar is now just a single 🏠 Home button (`#topNav` in `index.html`), not a flat row of chapter tabs. `tab-home` is the new default landing panel (`TABS[0]` in `walk.js`, `showTab('home')` at the end of the `window.addEventListener('load', ...)` init block). It shows two quick-action buttons, always visible above the pillar list regardless of pillar structure (`#homeStartWalkBtn` calls `showTab('walk')` + `window.startWalk()` directly; `#homeGenerateReportBtn` calls `showTab('report')` + `window.generateReport()` directly — no need to enter a pillar first) plus seven distinctly-styled top-level pillar accordions (`.home-block[data-block="land|preconstruction|design|field|scheduling|safety|financial"]`, each with its own accent color/icon-badge color, toggled open/closed by `.home-block-header` clicks adding/removing `.expanded`). An earlier same-day version had four Procore-style blocks instead of seven pillars — superseded before ship, don't resurrect the four-block grouping. Each pillar's `.home-block-chapters` list reveals the real chapter buttons one layer behind — `.home-chapter-btn[data-tab]`/`.home-admin-btn[data-tab]` still just call the pre-existing `showTab(name)` against the same `tab-*` panels below, nothing about the panels themselves changed. `disabled` chapter buttons with no `data-tab` (Land Acquisition, Ch.6 Land-to-Contract Pipeline/Victoria Land, Ch.9 Price-the-House, Ch.5 Invoicing, Ch.7 Consulting Business Rollout, Ch.8 Developer Dashboard) are placeholders — none of that functionality exists in the live app; Ch.6/Ch.9's real reference material is the parked `current/pipeline.ts`/`pipeline-view.tsx`, not anything live.
