...
- No build step. Push to `main` → Netlify (and GitHub Pages, while the repo is public) serves it directly, usually within a minute or two. Link-preview tags in `index.html` (`og:url`, `og:image`, `twitter:image`) point at the Netlify URL (2026-09-24, cache `sitewalk-v69`), so previews survive the repo going private.
