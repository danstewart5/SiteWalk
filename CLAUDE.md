...
## Photo tag suggestions + urgency-first report (2026-09-25, cache `sitewalk-v72`)
...
- **Photo tags.** Every walk photo (voice snap, Snap button, upload) goes through `requestPhotoTag()` → Worker `POST /tag-photo` (image shrunk to 768px, plus the voice trade and what was being said as hints). The reply `{kind: trade|safety|quality, trade, severity, confidence, observation}` is stored as `photo.suggestion`. Nothing changes until the user acts: a card (`photoTagCard()`) shows on the Walk screen (`#photoTagQueue`, newest only) and in Walk → Review (`#photoTagReviewList`, all), with trade / tag / urgency dropdowns prefilled from the suggestion, **Confirm** and **Keep as tagged**. The Review badge counts these too. A suggestion that wouldn't change anything is stored as `status: 'agrees'` and not shown.
...
- **Report order.** A new "Most Urgent First" block tops the report: every open Critical/High item across punch, COs, RFIs, Safety & Quality and Review, worst first. Each section below keeps its place but is now sorted by `byUrgency()`: open before closed, then severity, then capture order (was trade, then time). Each row has a coloured urgency badge; rows show "Photo flagged: Safety" / "Trade from photo" when a photo tag set them. Unlinked photos are sorted by their tag's urgency. The Walk Summary sorts within each trade the same way. The transcript/walk log stays in capture order (it's the record).
